...
class MotLoss(nn.Module):
	...
	def compute_orig_ospa_matching(self, outputs, targets, existance_threshold, c, p):
		...
		for i in range(bs):
			...
			if len(alive_output) > len(current_targets):
				...
				cardinality_error = c**p * cardinality_error
			elif len(current_targets) == 0:
				indices.append(([], []))
				loss += torch.Tensor([0])
			else:
				dist = torch.cdist(alive_output, current_targets, p=2)
				dist = dist.clamp_max(c)
				cost = torch.pow(input=dist, exponent=p)
				cost = cost.cpu()
				target_idx, output_idx = linear_sum_assignment(cost)
				indices.append((target_idx, output_idx))

				localization_error = 0
				for t, o in zip(target_idx, output_idx):
					localization_error += cost[t, o].item()
					# if cost[t, o] < self.cutoff_distance:
					# 	localization_cost += cost[t, o].item()
					# 	permutation_length += 1
				
				cardinality_error = abs(len(alive_output) - len(current_targets))
				cardinality_error = c**p * cardinality_error
				loss += torch.pow(input = torch.Tensor([(localization_error + cardinality_error) / len(current_targets)]), exponent = 1/p)

				localization_cost += (localization_error / len(current_targets))**(1/p)
				cardinality_cost += (cardinality_error / len(current_targets))**(1/p)
		...
		decomposition = {'localization': localization_cost, 'cardinality': cardinality_cost, 'n_matched_objs': permutation_length}
