...
def evaluate_metrics(data_generator, model, params, mot_loss, existance_threshold=0.75, num_eval=1000, verbose=False, data=None):
	...
	orig_ospa = {'random': {'total': [], 'localization': [], 'cardinality': []},
				 'output': {'total': [], 'localization': [], 'cardinality': []},
				 'true_meas': {'total': [], 'localization': [], 'cardinality': []}}
	prob_gospa = {'random': [], 'output': [], 'true_meas': []}
	detr = {'random': [], 'output': [], 'true_meas': []}
	# num_eval = 100
	for i in range(num_eval):
		t = time.time()
		if (i % (1+(num_eval // 100))) == 0:
		...
		if data_generator is None:
			...
			batch, labels, unique_ids = next(data_generator)
		...
		for i, key in enumerate(['output']):
			...
			oo, decompositionOSPA = mot_loss.compute_orig_ospa_matching(outs[i], labels, existance_threshold=params.loss.existence_prob_cutoff, c=2, p=1)
			...
			orig_ospa[key]['total'].append(oo.item())
			orig_gospa[key]['localization'].append(decomposition['localization'])
			orig_ospa[key]['localization'].append(decompositionOSPA['localization'])
			orig_ospa[key]['cardinality'].append(decompositionOSPA['cardinality'])
	...
	return orig_gospa, orig_ospa, prob_gospa, detr